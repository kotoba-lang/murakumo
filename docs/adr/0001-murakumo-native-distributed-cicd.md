...
Acceptance evidence, re-measured 2026-09-24 when this landed on main (#321,
sources renamed to `.cljk`; the JVM loader no longer reads them, so kbb is the
test host):

- `kbb -M:test -n <ns>` for each of the 34 CI/CD test namespaces: 33 green,
  83 tests / 289 assertions, zero failures/errors;
- `murakumo.cd-quic-live-test` (real localhost QUIC deployment RPC) does not
  load on kbb: `murakumo.overlay.quic-driver` is JVM-only (kwik), the same gap
  as the rest of the overlay. It is left failing, not skipped;
- checked-in coordinator and runner configurations validate.

Persistence notes from the port: blob bytes are written to the CAS by CID
and kept off the datom plane (`murakumo.blob`); tree and commit datoms persist
because arrangement 3d4fa5a admits vectors on its key path; on kbb a repo
snapshot is a Promise and the code after it goes through `murakumo.blob/then`.
