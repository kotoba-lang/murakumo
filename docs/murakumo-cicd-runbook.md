...
5. Seed the rollback bundle and all its component objects into the coordinator
   ...
   ```sh
   kbb -M:artifact-import /var/lib/murakumo/artifacts \
...
```sh
kbb -M:cd-node examples/cd-node.edn
kbb -M:ci-coordinator examples/ci-coordinator.edn
kbb -M:ci-runner examples/ci-runner.edn
```

These three talk to each other over the murakumo QUIC overlay
(`murakumo.overlay.quic-driver`), which today exists only on the JVM host
(kwik). On kbb their pure parts (config, identity, attestation, persistence,
the webhook handler) run and are tested, but serving or dialling the overlay
fails until that transport has a kbb host -- the same gap as the rest of the
overlay. The GitHub status publisher likewise needs the synchronous
babashka/JVM HTTP client and refuses on kbb rather than faking a post.
