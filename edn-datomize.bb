...
;;
;; このコピーは kotoba-lang/murakumo 用に schema-path のみ調整（manifest/ が無いので
;; リポジトリルート直下 schema.edn）。他のロジックは
;; com-junkawasaki/root の manifest/edn-datomize.bb と同一。
;;
;; 使用時の注意 (murakumo フェーズ3実測): wrap-map-keep-ns の promote-ns には、
;; 対象ファイルに既に存在するどの namespace とも衝突しない値を渡すこと
;; （例: cloud.edn は :cloud/* と :overlay/* を両方使っているので、裸キー
;; :relays/:policy の昇格先に "cloud" を使うと既存の :cloud/name 等と
;; 区別不能になる — "cloud-doc" のような別名にする）。消費側の reconstitute は
;; その promote-ns だけを剥がし、他の namespace はそのまま残す。
